...
data = json.loads((base / 'hybrid-mixed-sample.json').read_text())
...
for p in data['pages']:
    if p['sourceType']=='template':
        ...
        scene = f"<div class='scene' style='background:{bg.get(p['template'],'#eef2ff')}'><img class='avatar' src='{av}' alt='avatar'/><span class='chip'>template</span></div>"
    else:
        scene = f"<div class='scene real'><img class='realimg' src='{p['image']}' alt='real'/><span class='chip'>stock/ai</span></div>"
...
<title>Hybrid Mixed Sample</title>
<style>
...
(base / 'rendered-hybrid-mixed-sample.html').write_text(html)
print('rendered', base / 'rendered-hybrid-mixed-sample.html')
